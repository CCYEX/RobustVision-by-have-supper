#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""train_m0.py - 本机 4060 训练入口（排期 v2 本机路线）。

为什么是这个样子（而不是手写 PyTorch 循环）：
    Ultralytics 本身就是 PyTorch 的官方优化训练器（AMP 混合精度、mosaic 增强、
    余弦学习率等都已调好）。手写循环 = 放弃这些优化，只会更慢。
    ...
    3. cache="ram"    第一轮把图片缓存进内存（1.1 万张仅约 1GB），之后每轮免磁盘
...
if __name__ == "__main__":                     # Windows spawn 模式必须有主入口保护
    parser = argparse.ArgumentParser(description="本机 4060 训练（M0/M1/M2 通用）")
